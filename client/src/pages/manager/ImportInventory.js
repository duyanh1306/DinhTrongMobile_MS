...
export default function ImportInventory() {
    ...
    const loadInitData = async () => {
        ...
        if (data) {
            ...
            if (data.stores.length > 0) {
                setItemFormData(prev => ({ ...prev, storeId: data.stores[0]._id }));
                setPhoneFormData(prev => ({ ...prev, storeId: data.stores[0]._id }));
            }
        }
    };
    ...
    const handleAddBatchItem = (e) => {
        ...
        if (!itemFormData.storeId) return toast.warning("Vui lòng chọn Kho!");
        ...
        const newBatch = {
            ...
            storeName: stores.find(s => s._id === itemFormData.storeId)?.name
        };

        setPendingItemBatches([...pendingItemBatches, newBatch]);
        
        setItemFormData(prev => ({ ...prev, quantity: 1, ram: '', capacity: '', color: '', baseCost: 0, price: 0 }));
        setSelectedType(null);
        setFilterGroup("");
    ...
    const handleAddBatchPhone = (e) => {
        ...
        if (!phoneFormData.storeId) return toast.warning("Vui lòng chọn Kho!");
        ...
        const newBatch = {
            ...
            storeName: stores.find(s => s._id === phoneFormData.storeId)?.name
        };
                                ...
                                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                        <div>
                                            ...
                                            <select required value={itemFormData.storeId} onChange={e => setItemFormData({...itemFormData, storeId: e.target.value})} className="w-full border border-gray-300 p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
                            ...
                            <form onSubmit={handleAddBatchPhone}>
                                ...
                                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                        <div>
                                            <label className="block text-sm font-semibold mb-1">Kho nhận <span className="text-red-500">*</span></label>
                                            <select required value={phoneFormData.storeId} onChange={e => setPhoneFormData({...phoneFormData, storeId: e.target.value})} className="w-full border border-gray-300 p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
                                                {stores.map(s => <option key={s._id} value={s._id}>{s.name}</option>)}
                                            </select>
                                        </div>
                                        <div>
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex-1 flex flex-col h-full max-h-[80vh]">
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-100 flex-shrink-0">
                            ...
                            <button 
                                ...
                                disabled={loading || (pendingItemBatches.length === 0 && pendingPhoneBatches.length === 0)} 

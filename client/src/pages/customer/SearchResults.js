...
export default function SearchResults() {
    ...
    const [conditionFilter, setConditionFilter] = useState(null); // 'new' | 'used' | null
            ...
            try {
                ...
                const combinedData = filteredModels.map(model => {
                    ...
                    if (availablePhones.length > 0) {
                        startingPrice = Math.min(...availablePhones.map(p => p.sellingPrice || (p.importPrice * 1.15)));
                    }

                    let displayImage = model.image;
                    const phoneWithImage = availablePhones.find(p => p.specificImages && p.specificImages.length > 0);
            ...
            <div className="w-full pb-10">
                ...
                <div className="mb-6" ref={filterRef}>
                    ...
                    <div className="flex flex-wrap gap-2.5 relative">
                        ...
                        <button className="flex items-center gap-1.5 px-4 py-1.5 bg-white border border-blue-500 text-blue-600 rounded-lg text-sm font-medium hover:bg-blue-50 transition">
                            <Filter size={16} /> Bộ lọc
                        </button>

                        <button 
                            onClick={() => setInStockOnly(!inStockOnly)}
                            className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-lg text-sm transition ${inStockOnly ? 'bg-blue-50 border-blue-500 text-blue-600 shadow-sm' : 'bg-white border-gray-200 text-gray-700 hover:border-blue-400'}`}
                        >
                            <Truck size={16}/> Sẵn hàng
                        </button>

                        {/* LỌC HÀNG MỚI */}
                        <button 
                            onClick={() => setConditionFilter(conditionFilter === 'new' ? null : 'new')}
                            className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-lg text-sm transition ${conditionFilter === 'new' ? 'bg-blue-50 border-blue-500 text-blue-600 shadow-sm' : 'bg-white border-gray-200 text-gray-700 hover:border-blue-400'}`}
                        >
                            <CheckCircle size={16}/> Hàng Mới
                        </button>

                        {/* LỌC HÀNG CŨ */}
                        <button 
                            onClick={() => setConditionFilter(conditionFilter === 'used' ? null : 'used')}
                            className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-lg text-sm transition ${conditionFilter === 'used' ? 'bg-blue-50 border-blue-500 text-blue-600 shadow-sm' : 'bg-white border-gray-200 text-gray-700 hover:border-blue-400'}`}
                        >
                            <RotateCcw size={16}/> Hàng Cũ
                        </button>
                        ...
                        <div className="relative">
                            <button 
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-lg text-sm transition ${(appliedPrice.min || appliedPrice.max || activeDropdown === 'price') ? 'bg-blue-50 border-blue-500 text-blue-600 shadow-sm' : 'bg-white border-gray-200 text-gray-700 hover:border-blue-400'}`}
                            >
                                Xem theo giá <ChevronDown size={14}/>
                            </button>
                                ...
                                <div className="absolute top-full left-0 mt-2 w-72 bg-white rounded-xl shadow-2xl border border-gray-100 p-4 z-50 animate-in fade-in zoom-in duration-200">
                                    ...
                                    <div className="flex items-center gap-2 mb-4">
                                        <input 
                                            ...
                                            value={tempPrice.min} onChange={e => setTempPrice({...tempPrice, min: e.target.value})}
                                        ...
                                        <input 
                                            ...
                                            value={tempPrice.max} onChange={e => setTempPrice({...tempPrice, max: e.target.value})}
                                            className="w-full border rounded-lg px-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-blue-500"
                                        />
                                    </div>
                        ...
                        {availableStorages.length > 0 && (
                            <div className="relative">
                                <button 
                                    ...
                                    className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-lg text-sm transition ${(selectedStorages.length > 0 || activeDropdown === 'storage') ? 'bg-blue-50 border-blue-500 text-blue-600 shadow-sm' : 'bg-white border-gray-200 text-gray-700 hover:border-blue-400'}`}
                                >
                                    Bộ nhớ trong <ChevronDown size={14}/>
                                </button>
                                        ...
                                        <div className="grid grid-cols-2 gap-2 mb-4">
                                            {availableStorages.map(storage => (
                                                <button 
                                                    ...
                                                    className={`py-1.5 border rounded-lg text-sm transition ${selectedStorages.includes(storage) ? 'border-blue-500 bg-blue-50 text-blue-700 font-semibold' : 'border-gray-200 text-gray-600 hover:border-blue-300'}`}
                                                >
                                                    {storage}
                                                </button>
                                            ))}
                ...
                {/* --- ĐANG LỌC THEO --- */}
                {(inStockOnly || conditionFilter || appliedPrice.min || appliedPrice.max || selectedStorages.length > 0) && (
                    <div className="flex flex-wrap items-center gap-2 mb-6">
                        <span className="text-sm font-semibold text-gray-700 mr-2">Đang lọc theo:</span>
                        
                        {inStockOnly && (
                            <div className="flex items-center gap-1 bg-blue-50 border border-blue-200 text-blue-700 px-3 py-1 rounded-full text-xs font-medium">
                                Sẵn hàng <button onClick={() => removeFilter('inStock')} className="hover:bg-blue-200 rounded-full p-0.5"><X size={12}/></button>
                            </div>
                        )}

                        {conditionFilter && (
                            <div className="flex items-center gap-1 bg-blue-50 border border-blue-200 text-blue-700 px-3 py-1 rounded-full text-xs font-medium">
                                {conditionFilter === 'new' ? 'Hàng Mới' : 'Hàng Cũ'} 
                                <button onClick={() => removeFilter('condition')} className="hover:bg-blue-200 rounded-full p-0.5"><X size={12}/></button>
                            </div>
                        )}
                        
                        {(appliedPrice.min || appliedPrice.max) && (
                            <div className="flex items-center gap-1 bg-blue-50 border border-blue-200 text-blue-700 px-3 py-1 rounded-full text-xs font-medium">
                                Giá: {appliedPrice.min ? `${Number(appliedPrice.min).toLocaleString()}đ` : '0đ'} - {appliedPrice.max ? `${Number(appliedPrice.max).toLocaleString()}đ` : 'Max'}
                                <button onClick={() => removeFilter('price')} className="hover:bg-blue-200 rounded-full p-0.5"><X size={12}/></button>
                            </div>
                        )}

                        {selectedStorages.length > 0 && (
                            <div className="flex items-center gap-1 bg-blue-50 border border-blue-200 text-blue-700 px-3 py-1 rounded-full text-xs font-medium">
                                Bộ nhớ: {selectedStorages.join(' | ')}
                                <button onClick={() => removeFilter('storage')} className="hover:bg-blue-200 rounded-full p-0.5"><X size={12}/></button>
                            </div>
                        )}
                        
                        <button onClick={() => {setInStockOnly(false); setConditionFilter(null); setAppliedPrice({min:"", max:""}); setSelectedStorages([]);}} className="text-sm text-red-500 hover:text-red-600 font-medium ml-2 transition">Xóa tất cả</button>
                    </div>
                )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                        {filteredProducts.map((product) => (
                            <Link key={product._id} to={`/product/${product._id}`} className="bg-white p-4 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col h-full group relative overflow-hidden">
                                ...
                                <div className={`h-48 flex items-center justify-center p-2 mb-4 mt-4 ${product.stockCount === 0 ? 'opacity-40 grayscale' : ''}`}>

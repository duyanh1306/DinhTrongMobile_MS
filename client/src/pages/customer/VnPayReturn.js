...
export default function VnPayReturn() {
    ...
    useEffect(() => {
        ...
        if (responseCode === '00') {
            setStatus('success');
            // Gọi API dọn dẹp sạch giỏ hàng sau khi mua thành công
                ...
                <div className="bg-white p-8 md:p-12 rounded-3xl shadow-xl max-w-lg w-full text-center border border-gray-100">
                    ...
                    {status === 'success' && (
                        <div className="flex flex-col items-center animate-in zoom-in duration-300">
                            ...
                            <p className="text-gray-500 mb-6 text-sm">Cảm ơn bạn đã mua sắm tại cửa hàng. Đơn hàng của bạn đang được chuẩn bị.</p>
                            
                            <div className="bg-gray-50 w-full p-5 rounded-2xl mb-8 border border-gray-100 text-left">
                                <div className="flex justify-between items-center mb-3 pb-3 border-b border-gray-200">
                                    <span className="text-sm text-gray-500 font-semibold">Số tiền thanh toán:</span> 
                                    <strong className="text-red-600 text-xl">{formatMoney(amount)}</strong>
                                </div>
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-sm text-gray-500">Mã giao dịch (VNPay):</span> 
                                    <strong className="text-gray-800 font-mono">{searchParams.get('vnp_TransactionNo')}</strong>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-gray-500">Mã tham chiếu đơn:</span> 
                                    <strong className="text-gray-800 font-mono">{searchParams.get('vnp_TxnRef')}</strong>
                                </div>
                            </div>
                    ...
                    {status === 'failed' && (
                        <div className="flex flex-col items-center animate-in zoom-in duration-300">
                            ...
                            <p className="text-gray-500 mb-8 text-sm">Giao dịch đã bị hủy hoặc có lỗi xảy ra từ phía ngân hàng. Đơn hàng của bạn đã được lưu nháp, vui lòng thử lại.</p>

        ...
        <table className="w-full text-left border-collapse">
            ...
            {details.map((d, idx) => {
                ...
                <td className="py-2 pr-1">
                    ...
                    {d.phoneId
                        ? `Mã: ${d.phoneId._id ? d.phoneId._id.substring(d.phoneId._id.length - 6).toUpperCase() : d.phoneId.substring(d.phoneId.length - 6).toUpperCase()}`
                        : `SN: ${d.itemId?.serialCode || d.identifier}`}
                    </div>
...
export default function SalePOS() {
  ...
  const fetchInventory = async () => {
    ...
    try {
      ...
      const formattedPhones = (Array.isArray(phonesData) ? phonesData : phonesData.data || []).map(p => ({ 
        ...p, isPhone: true, displayPrice: p.sellingPrice, displayName: p.phoneModelId?.name,
        shortCode: p._id.substring(p._id.length - 6).toUpperCase(), identifier: `Mã: ${p._id.substring(p._id.length - 6).toUpperCase()}` 
      }));
      const formattedItems = (Array.isArray(itemsData) ? itemsData : itemsData.data || []).map(i => ({ 
        ...i, isPhone: false, displayPrice: i.price, displayName: i.name || i.item_type?.name, identifier: `SN: ${i.serialCode}`
      }));
  ...
  const handleScan = (e) => {
    if (e.key === "Enter") {
      ...
      if (!code) return;
      const foundProduct = inventory.find(item => item.isPhone ? (item.shortCode === code || item._id.toUpperCase() === code) : (item.serialCode.toUpperCase() === code));
  ...
  const addToCart = (product) => {
    setCart([...cart, { ...product, phoneId: product.isPhone ? product._id : null, itemId: !product.isPhone ? product._id : null, price: product.displayPrice, name: product.displayName, identifier: product.identifier }]);
                      ...
                      <tr key={idx} className="border-t hover:bg-gray-50">
                        <td className="p-3">
                          ...
                          <div className="text-[10px] text-gray-400 ml-6">
                            {detail.phoneId ? `Mã: ${detail.phoneId._id ? detail.phoneId._id.substring(detail.phoneId._id.length - 6).toUpperCase() : detail.phoneId.substring(detail.phoneId.length - 6).toUpperCase()}` : `SN: ${detail.itemId?.serialCode || detail.identifier}`}
                          </div>
                        </td>

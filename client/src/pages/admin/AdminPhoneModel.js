...
export default function AdminPhoneModel() {
    ...
    const [activeModalTab, setActiveModalTab] = useState('basic'); // 'basic' or 'compatible'
    const [itemTypes, setItemTypes] = useState([]); // For compatible item types
    const [itemTypeSearch, setItemTypeSearch] = useState(''); // Search for item types
    const [itemTypePagination, setItemTypePagination] = useState({
    ...
    // Filter item types based on search and pagination
...
// Calculate pagination info for filtered results
    ...
    const handleAddPhoneModel = () => {
        ...
        setEditingId(null);
        setActiveModalTab('basic'); // Reset to basic tab 
        setShowModal(true);
    };

    const handleEditPhoneModel = (model) => {
        console.log("Editing phone model:", model); // Debug log
        console.log("Compatible item types:", model.compatibleItemTypes); // Debug log

        // Extract just the IDs from the populated compatibleItemTypes
        const compatibleItemIds = model.compatibleItemTypes?.map(item =>
            item._id || item
        ) || [];

        console.log("Extracted compatible item IDs:", compatibleItemIds); // Debug log
        setIsEditing(true);
    ...
    return (
        <div className="flex flex-col h-full space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3"><Smartphone className="text-blue-600" size={28}/><h1
                    className="text-2xl font-bold text-gray-800">Quản lý Danh mục Điện Thoại</h1></div>
                ...
                <div className="overflow-x-auto flex-1">
                    <table className="w-full text-left">
                        ...
                        <tr>
                            ...
                            <th className="px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase">Thao
                                tác
                            </th>
                        </tr>
                        </thead>
                        ...
                        {phoneModels.length === 0 ? (
                            <tr>
                                <td colSpan="5" className="px-6 py-12 text-center text-gray-500">Không tìm thấy dữ
                                    liệu
                                </td>
                            </tr>
                        ) : (
                            phoneModels.map(model => (
                                <tr key={model._id} className="hover:bg-blue-50/50 transition">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <button onClick={() => handleEditPhoneModel(model)}
                                                className="text-blue-600 bg-blue-50 p-2 rounded-lg hover:bg-blue-100 transition">
                                            <Edit size={16}/></button>
                                    </td>
                                </tr>
                            ))
                    ...
                    <div
                        ...
                        {isEditing && (
                            <div className="flex border-b">
                                ...
                                <button
                                    type="button"
                                    onClick={() => setActiveModalTab('compatible')}
                                    className={`px-6 py-3 font-medium ${
                                        activeModalTab === 'compatible'
                                            ? 'border-b-2 border-blue-500 text-blue-600'
                                            : 'text-gray-500 hover:text-gray-700'
                                    }`}
                                >
                        ...
                        <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-6">
                            <div className="p-6 space-y-6">
                                {/* Show basic info tab OR all content when not editing */}
                                {!isEditing || activeModalTab === 'basic' ? (
                                    <>
                                        {/* Keep ALL your existing form content here */}
                                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                            <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4">
                                                <div><label className="block text-sm font-semibold mb-1">Tên máy</label><input
                                                    type="text" name="name" value={formData.name}
                                                    onChange={handleInputChange} required
                                                    className="w-full border p-2 rounded outline-none focus:ring-2 focus:ring-blue-500"/>
                                                </div>
                                                <div><label
                                                    className="block text-sm font-semibold mb-1">Hãng</label><input
                                                    type="text" name="brand" value={formData.brand}
                                                    onChange={handleInputChange} required
                                                    className="w-full border p-2 rounded outline-none focus:ring-2 focus:ring-blue-500"/>
                                                </div>
                                                <div><label className="block text-sm font-semibold mb-1">Độ mới
                                                    (0-1)</label><input type="number" name="condition"
                                                                        value={formData.condition} step="0.01"
                                                                        onChange={handleInputChange}
                                                                        className="w-full border p-2 rounded outline-none focus:ring-2 focus:ring-blue-500"/>
                                                </div>
                                            </div>
                                            <div
                                                className="border rounded-lg p-3 text-center bg-gray-50 flex flex-col items-center justify-center">
                                                {formData.previewImage ? <img src={formData.previewImage} alt="preview"
                                                                              className="h-20 w-auto mb-2 object-contain"/> :
                                                    <ImageIcon className="h-10 w-10 text-gray-300 mb-2"/>}
                                                <input type="file" accept="image/*" onChange={handleFileChange}
                                                       className="text-xs w-full text-center ml-4"/>
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div
                                                className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-gray-50 p-4 rounded-xl border">
                                                {[
                                                    {l: "Màn hình", n: "spec_screenSize"}, {
                                                        l: "Tấm nền",
                                                        n: "spec_screenTechnology"
                                                    },
                                                    {l: "Camera sau", n: "spec_rearCamera"}, {
                                                        l: "Camera trước",
                                                        n: "spec_frontCamera"
                                                    },
                                                    {l: "Chip", n: "spec_chipset"}, {
                                                        l: "Bộ nhớ",
                                                        n: "spec_internalStorage"
                                                    },
                                                    {l: "Pin/Sạc", n: "spec_cpu"}, {l: "Hệ điều hành", n: "spec_os"},
                                                ].map(f => (
                                                    <div key={f.n}>
                                                        <label className="text-xs text-gray-600">{f.l}</label>
                                                        <input type="text" name={f.n}
                                                               value={formData.specifications[f.n.replace('spec_', '')]}
                                                               onChange={handleInputChange}
                                                               className="w-full border p-1.5 rounded text-sm outline-none focus:ring-2 focus:ring-blue-400"/>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="border rounded-lg overflow-hidden">
                                            <table className="w-full">
                                                ...
                                                <tr>
                                                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Linh
                                                        kiện
                                                    </th>
                                                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Tương
                                                        thích
                                                    </th>
                                                </tr>
                                                </thead>
                                                ...
                                                {filteredItemTypes.map((itemType) => (
                                                    <tr key={itemType._id}>
                                                        ...
                                                        <td className="px-4 py-3">
                                                            <input
                                                                type="checkbox"
                                                                checked={formData.compatibleItemTypes?.some(id =>
                                                                    id.toString() === itemType._id.toString()
                                                                ) || false}
                                                                onChange={(e) => {
                                                                    console.log("Checkbox change:", itemType._id, e.target.checked); // Debug log
                                                                    console.log("Current compatibleItemTypes:", formData.compatibleItemTypes); // Debug log
                                                                    if (e.target.checked) {
                                                                        setFormData(prev => ({
                                                                            ...prev,
                                                                            compatibleItemTypes: [...(prev.compatibleItemTypes || []), itemType._id]
                                                                        }));
                                                                    } else {
                                                                        setFormData(prev => ({
                                                                            ...prev,
                                                                            compatibleItemTypes: (prev.compatibleItemTypes || []).filter(id => id !== itemType._id)
                                                                        }));
                                                                    }
                                                                }}
                                        ...
                                        {itemTypeTotalPages > 1 && (
                                            <div className="flex items-center justify-between mt-4">
                                                <div className="flex items-center space-x-2">
                                                    ...
                                                    <select
                                                        value={itemTypePagination.itemsPerPage}
                                                        onChange={(e) => handleItemTypeItemsPerPageChange(e.target.value)}
                                                ...
                                                <div className="flex items-center space-x-1">
                                                    <button
                                                        onClick={() => handleItemTypePageChange(itemTypePagination.currentPage - 1)}
                                                        disabled={itemTypePagination.currentPage === 1}
                                                    ...
                                                    {Array.from({length: Math.min(5, itemTypeTotalPages)}, (_, i) => {
                                                        let pageNum;
                                                        if (itemTypeTotalPages <= 5) {
                                                            pageNum = i + 1;
                                                        } else if (itemTypePagination.currentPage <= 3) {
                                                            pageNum = i + 1;
                                                        } else if (itemTypePagination.currentPage >= itemTypeTotalPages - 2) {
                                                            pageNum = itemTypeTotalPages - 4 + i;
                                                        } else {
                                                            pageNum = itemTypePagination.currentPage - 2 + i;
                                                        }

                                                        return (
                                                            <button
                                                                key={pageNum}
                                                                onClick={() => handleItemTypePageChange(pageNum)}
                                                                className={`px-3 py-1 text-sm border rounded ${
                                                                    itemTypePagination.currentPage === pageNum
                                                                        ? 'bg-blue-600 text-white border-blue-600'
                                                                        : 'border-gray-300 hover:bg-gray-50'
                                                                }`}
                                                            >
                                                    ...
                                                    <button
                                                        onClick={() => handleItemTypePageChange(itemTypePagination.currentPage + 1)}
                                                        disabled={itemTypePagination.currentPage === itemTypeTotalPages}
                                                        className="px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                                                    >
                                                        Sau
                                                    </button>
                            ...
                            <div className="flex justify-end gap-3 pt-4 border-t">
                                <button type="button" onClick={handleCloseModal}
                                        className="px-4 py-2 text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition">Hủy
                                </button>
                                <button type="submit"
                                        className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">{isEditing ? 'Cập nhật' : 'Thêm'}</button>
                            </div>
                        </form>
                    </div>
                </div>

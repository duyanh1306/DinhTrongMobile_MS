    ...
    <div className="bg-white p-6 rounded-xl border border-red-100 shadow-sm">
      <div className="flex justify-between items-center mb-4 pb-4 border-b border-red-100">
        <label className="font-bold text-gray-800 flex items-center gap-2 text-lg">
          <Package size={22} className="text-red-600" /> Khai báo linh kiện tháo
          rời
        </label>
      </div>
      ...
      <div className="mb-6">
        <p className="text-sm font-bold text-gray-700 mb-3">
          Linh kiện đạt chuẩn 100% (Bấm để bóc):
        </p>
        <div className="flex flex-wrap gap-2">
          {goodParts.map((item, idx) => {
            ...
            if (isExtracted) {
              return (
                <span
                  key={idx}
                  className="px-4 py-2 border border-green-200 bg-green-50 text-green-600 rounded-xl text-xs font-bold flex items-center gap-1 opacity-60 cursor-not-allowed"
                >
          ...
          {goodParts.length === 0 && (
            <span className="text-sm text-gray-400 italic">
              Máy nát bét, không có linh kiện nào tái sử dụng được.
            </span>
          )}
        </div>
      </div>
        ...
        {dismantleParts.map((part, idx) => (
          ...
          >
            <button
              ...
              className="absolute top-4 right-4 text-gray-400 hover:text-white bg-white hover:bg-red-500 border p-2 rounded-full transition shadow-sm z-10"
            >
              <Trash2 size={16} />
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mr-12 mb-5">
              <div>
                ...
                <select
                  disabled
                  className="w-full p-2.5 bg-gray-100 border border-gray-200 rounded-lg outline-none text-gray-600 cursor-not-allowed font-medium"
                >
                  <option value="">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-white border rounded-lg outline-none focus:ring-2 focus:ring-red-500 text-sm font-bold text-green-600"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-600 uppercase mb-1.5">
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-white border rounded-lg outline-none focus:ring-2 focus:ring-red-500 text-sm"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-600 uppercase mb-1.5">
